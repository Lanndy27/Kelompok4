    ...
    <div className="mt-[93px] flex flex-col items-center px-[129px]">
      <div className="container-xs flex flex-col gap-24 md:gap-[72px] md:px-5 sm:gap-12">
        <div className="flex items-center justify-between gap-10 border border-solid border-black bg-white-a700 md:flex-row mb-[66px] ">
          ...
          />
          <div className="flex flex-col items-start gap-11 md:px-5">
            {" "}
            {/* Kontainer teks dan ChipView */}
            <div className="flex flex-col items-start gap-11">
              {" "}
              {/* Elemen teks */}
              <Heading
                as="h2"
                className="text-[48px] font-bold text-blk md:text-[44px] sm:text-[38px]"
              >
                Tips Penggunaan Aksesoris
              </Heading>
              <Text
                as="p"
                className="w-[84%] text-[18px] font-normal leading-7 text-blk md:w-full"
              >
                Penggunaan aksesoris dalam gaya berpenampilan pria bisa
                bermanfaat untuk mempermanis tampilan agar menjadi lebih stylish
                dan semakin keren. Penambahan eksesoris dalam penampilan
                sebaiknya semua seimbang, yang terpenting kamu harus merasa
                nyaman dan lebih percaya diri. Dan berikut beberapa aksesoris
                wajib yang harus dimiliki pria agar bisa tampil semakin stylish.{" "}
              </Text>
            </div>
            <ChipView
              options={ChipOptions}
                ...
                <React.Fragment key={option.index}>
                  {option.isSelected ? (
                    <div
                      ...
                      className="flex h-[32px] min-w-[186px] cursor-pointer flex-row items-center justify-center border border-solid border-blk border-white-a700_99 bg-white-a700 px-2.5 text-center text-[18px] text-blk"
                    >
